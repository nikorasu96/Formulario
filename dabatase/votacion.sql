...
select  *   from
  votos;
SELECT
  votos.nombre_apellido,
  votos.alias,
  votos.rut,
  votos.email,
  regiones.nombre AS nombre_region,
  comunas.nombre AS nombre_comuna,
  candidatos.nombre_apellido AS nombre_candidato,
  votos.como_se_entero
FROM
  votos
  JOIN regiones ON votos.region_id = regiones.id
  JOIN comunas ON votos.comuna_id = comunas.id
...
CREATE TABLE votos (
    ...
    -- Agregamos la columna 'email' aquí.
